### 5/13/25, EB: Here I'm tweaking the autoencoder mapping script from AD in /Autoencoder/Efficacy/accuracy_maps.py
### to fit the results from my autoencoder model.
...
PREDICTIONS_PATH = 'County Classification/AE_Preds/ae_mortality_errors.csv'
PREDICTIONS_NAMES = [f'{year} Preds' for year in range(2011, 2023)]

# New save path
SAVE_DIR = 'County Classification/AE_Plots'
os.makedirs(SAVE_DIR, exist_ok=True)  # Automatically create if it doesn't exist
...
def load_reconstruction_errors(error_path):
    error_df = pd.read_csv(error_path, dtype={'Year': str})
    error_df = error_df.set_index('Year')
    error_df.columns = error_df.columns.astype(str)
    return error_df
...
def calculate_accuracy_from_errors(error_df, year):
    year = str(year)
    year_errors = error_df.loc[year].copy()

    # Convert errors to accuracy (normalize inversely)
    max_err = year_errors.max()
    accuracy = 1 - (year_errors / max_err) if max_err > 0 else np.ones_like(year_errors)

    # Normalize to fit within colormap
    accuracy = accuracy.apply(lambda x: 0.9999 if x == 1 else (0.0001 if x == 0 else x))

    acc_df = pd.DataFrame({'FIPS': year_errors.index, f'{year} Accuracy': accuracy})
    acc_df['FIPS'] = acc_df['FIPS'].astype(str).str.zfill(5)
    return acc_df
...
def construct_accuracy_map(shape, year):
    ...
    plt.savefig(os.path.join(SAVE_DIR, f"{year}_ae_acc_map.png"), bbox_inches=None, pad_inches=0, dpi=300)
...
def main():
    shape = load_shapefile(SHAPE_PATH)
    error_df = load_reconstruction_errors(PREDICTIONS_PATH)

    for year in range(2010, 2023):
        acc_df = calculate_accuracy_from_errors(error_df, year)
        shape_merged = merge_data_shape(shape, acc_df)
        construct_accuracy_map(shape_merged, str(year))
        print(f'Plot printed for {year}.')
