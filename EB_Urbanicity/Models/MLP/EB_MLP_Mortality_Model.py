...
def run_mlp_pipeline_multiyear_train():
    ...
    results_df.to_csv(os.path.join(RESULTS_DIR, 'PyTorch_MLP_model_results_13.7.3.1.csv'), index=False)
    all_fold_results_df.to_csv(os.path.join(RESULTS_DIR, 'PyTorch_MLP_model_preds_13.7.3.1.csv'), index=False)
    print(f'✅ Saved PyTorch MLP results to {RESULTS_DIR}/PyTorch_MLP_model_results_13.7.3.1.csv')
    print(f'✅ Saved PyTorch MLP predictions to {RESULTS_DIR}/PyTorch_MLP_model_preds_13.7.3.1.csv')
...
def plot_error_histograms_multiyear_train():
    ...
    for year in years:
        ...
        out_path = os.path.join(OUT_DIR, f'Unified_MLP_Error_Histogram_Year_{year}_13.7.3.1.png')
        plt.savefig(out_path, dpi=300)
        plt.close()
        print(f"✅ Saved: {out_path}")
