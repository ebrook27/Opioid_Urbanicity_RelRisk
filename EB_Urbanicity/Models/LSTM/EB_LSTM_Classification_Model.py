### 7/21/25, EB: Talked with Andrew today, he clarified what Adam was suggesting in our last meeting. Adam suggested we predict the risk level, but we should approach it as a classification problem,
### and instead of using the log-normal distribution, we should use the empirical distribution of the mortality to defin our classes.
### So what I'm trying to do here is use the LSTM pipeline we've been working on to do this. I will need to use the empirical distribution of the mortality to define the classes, and then modify
### the LSTM framework to handle classification instead of regression.

### 7/24/25, EB: Results are pretty terrible, I think 10 categories is tough to predict. I'm going to try a few different approaches to improve things:
### 1. Use class weightings, since the data is very zero-inflated.
### 2. Change the architecture to include more dropout and early stopping.

### 8/2/25, EB: Talked with Andrew, showed him the poor results. He told me he did some analysis and realized that when useing the empirical distribution, the classes are very unbalanced. In some years, 
### the bottom 30% of mortality is entirely 0, which makes the bottom 3 classes in my model identical, and hence not really learnable. We had the idea of using a 0 class for the zero mortality counties, and then using deciles for the rest.
### That turns this into an 11-class classification problem, which on its face might be harder to learn, but I can't really think of a better way to approach this problem right now, without getting really into the weeds.
### The functions preprocess_with_zero_class() and build_lstm_classification_sequences() are designed to implement this idea. I also had to switch the LSTMClassifier to have an 11 class output instead of 10.
...
def main():
    ...
    X_seq, X_static, y, fips_arr, year_arr = build_lstm_classification_sequences(
        df_processed, svi_variables=DATA[1:], sequence_length=3
    )
