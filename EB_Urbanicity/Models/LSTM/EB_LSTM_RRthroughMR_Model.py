### 8/28/25, EB: Man. Nothing is doing a good job of predicting the relative risk scores. It's such a weird quantity, because it's a ratio of two ratios.
### Predicting it directly using binned mortality rates, whether that be through multi-class classfication approaches (whether using continuous or categorical
### inputs), logistic ordinal regression models, that binary decomposition of ordinal categories (logistic or XGB), or my custom all-threshold ordinal XGB,
### the results were never great. I got them to be not terrible, with Spearman correlation around 0.68ish and a top 10% hit rate of between 40-50%, but this
### is pretty lackluster to me.
### My latest thought is similar to one I had a while ago: leverage our pretty good mortality rate prediction by converting it into a relative risk score afterwards.
### IOW, use the LSTM (or other model, starting with LSTM) to predict mortality rates year on year. Then we do the two-stage Log-Normal fitting (bin up zeros, fit LN to rest)
### to define risk levels, and calculate scores from there. Then we can compare predicted scores to actual scores each year.
### If this doesn't work, I'm truly out of ideas.
### To start this, I'm copying the working categorical LSTM code below, so I can tweak it to fit this new purpose.
